...
from services.zootecnia import get_age_display, get_age_category
...
def _campo_animal():
    ...
    cat=get_age_category(animal.get("birth_date"))
    ...
    with st.expander("✏️ Corrigir / redefinir idade"):
        ...
        if st.button("💾 Salvar nova idade", key=f"save_age_{aid}", type="primary"):
            if err2:
                ...
                st.success(f"✅ Idade atualizada · Categoria: **{get_age_category(bd2)}**")
                st.rerun()
...
def page_lotes():
    ...
    with lt1:
        _render_tab_visao_geral(lotes)

    with lt2:
        _render_tab_novo_lote()

    with lt3:
        _lotes_transferir_animais(lotes)


# ─── NDVI por Piquete (Spec 0079) ───────────────────────────────────────────
AVISO_NDVI_MATERIA_SECA = (
    "ℹ️ NDVI não equivale a matéria seca: nenhum dado de satélite substitui a "
    "avaliação presencial do pasto. Transformar índice vegetativo em "
    "estimativa de forragem requer calibração de campo com medições reais."
)


@st.cache_data(ttl=86400, show_spinner=False)
def _consultar_ndvi_cacheado(
    anel_tuplas: tuple[tuple[float, float], ...],
    inicio: date,
    fim: date,
            ...
            if modo == "kg":
                ...
                if sel_animals:
                    sug = db.get_expected_price_kg(
                        get_age_category(sel_animals[0].get("birth_date")), sel_animals[0]["sex"])
    ...
    with st.form("f_cad",clear_on_submit=False):
        ...
        if st.form_submit_button("✅ Cadastrar Animal",type="primary",use_container_width=True):
            ...
            if errs:
                ...
                cat = get_age_category(birth_date_str)
...
def _casos_do_rebanho():
    ...
    for a in db.get_all_animals(status="ativo"):
        casos.append({
            "id": a["id"],
            "especie": "bovino",
            "categoria": get_age_category(a.get("birth_date"), a.get("sex")),
            "sexo": a.get("sex"),
