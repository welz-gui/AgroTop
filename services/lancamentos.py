"""Serviço de normalização de lançamentos financeiros (função pura).

Converte registros de `sales`, `fixed_costs`, `animal_costs` e
`insumo_transactions` para o formato único consumido por `services.caixa`.
"""

from typing import Iterable, Any, Dict, Optional


def _safe_float(val: Any, default: float = 0.0) -> float:
    if val is None:
        return default
    try:
        return float(val)
    except (ValueError, TypeError):
        return default


def _safe_str(val: Any, default: str = "") -> str:
    if val is None:
        return default
    return str(val)


def _normalizar_venda(v: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not isinstance(v, dict):
        return None
    dt = v.get("sale_date") or v.get("data") or v.get("date")
    dt_str = _safe_str(dt, "") or None
    raw_v = v.get("total_value")
    raw_valor = raw_v if raw_v is not None else v.get("valor")
    return {
        "tipo": "receita",
        "valor": _safe_float(raw_valor),
        "categoria": "venda",
        "competencia": dt_str,
        "vencimento": dt_str,
        "pagamento": dt_str,
    }


def _normalizar_custo_fixo(cf: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not isinstance(cf, dict):
        return None
    dt = cf.get("cost_date") or cf.get("data") or cf.get("date")
    dt_str = _safe_str(dt, "") or None
    raw_a = cf.get("amount")
    raw_amount = raw_a if raw_a is not None else cf.get("valor")
    raw_c = cf.get("category")
    raw_cat = raw_c if raw_c is not None else cf.get("categoria")
    return {
        "tipo": "despesa",
        "valor": _safe_float(raw_amount),
        "categoria": _safe_str(raw_cat, ""),
        "competencia": dt_str,
        "vencimento": dt_str,
        "pagamento": dt_str,
    }


def _normalizar_custo_animal(ca: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not isinstance(ca, dict):
        return None
    dt = ca.get("cost_date") or ca.get("data") or ca.get("date")
    dt_str = _safe_str(dt, "") or None
    raw_a = ca.get("amount")
    raw_amount = raw_a if raw_a is not None else ca.get("valor")
    raw_c = ca.get("cost_type")
    raw_cat = raw_c if raw_c is not None else ca.get("categoria")
    return {
        "tipo": "despesa",
        "valor": _safe_float(raw_amount),
        "categoria": _safe_str(raw_cat, ""),
        "competencia": dt_str,
        "vencimento": dt_str,
        "pagamento": dt_str,
    }


def _normalizar_compra_insumo(ci: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not isinstance(ci, dict):
        return None
    tipo_t = _safe_str(ci.get("type") or ci.get("tipo"), "").strip().lower()
    if tipo_t != "compra":
        return None

    dt = ci.get("transaction_date") or ci.get("data") or ci.get("date")
    dt_str = _safe_str(dt, "") or None

    raw_q = ci.get("quantity")
    raw_qty = raw_q if raw_q is not None else ci.get("quantidade")
    qty = _safe_float(raw_qty, 0.0)

    insumo_info = ci.get("insumo")
    if isinstance(insumo_info, dict):
        cpu = insumo_info.get("cost_per_unit")
        raw_cpu = cpu if cpu is not None else insumo_info.get("custo_unitario")
        cost_per_unit = _safe_float(raw_cpu, 0.0)
        cat = _safe_str(insumo_info.get("name") or insumo_info.get("nome"), "")
    else:
        cost_per_unit = 0.0
        cat = _safe_str(ci.get("category") or ci.get("categoria"), "")

    valor = qty * cost_per_unit

    return {
        "tipo": "despesa",
        "valor": valor,
        "categoria": cat,
        "competencia": dt_str,
        "vencimento": dt_str,
        "pagamento": dt_str,
    }


def _normalizar_conta_pagar(cp: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not isinstance(cp, dict):
        return None
    return {
        "tipo": "despesa",
        "valor": _safe_float(cp.get("valor")),
        "categoria": _safe_str(cp.get("descricao"), "conta a pagar"),
        "competencia": None,
        "vencimento": _safe_str(cp.get("vencimento"), "") or None,
        "pagamento": _safe_str(cp.get("data_pagamento"), "") or None,
    }


def _normalizar_conta_receber(cr: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    if not isinstance(cr, dict):
        return None
    return {
        "tipo": "receita",
        "valor": _safe_float(cr.get("valor")),
        "categoria": _safe_str(cr.get("descricao"), "conta a receber"),
        "competencia": None,
        "vencimento": _safe_str(cr.get("vencimento"), "") or None,
        "pagamento": _safe_str(cr.get("data_recebimento"), "") or None,
    }


def normalizar(
    *,
    vendas: Iterable[dict] = (),
    custos_fixos: Iterable[dict] = (),
    custos_animal: Iterable[dict] = (),
    compras_insumo: Iterable[dict] = (),
    contas_pagar: Iterable[dict] = (),
    contas_receber: Iterable[dict] = (),
) -> list[dict]:
    """Normaliza lançamentos para a estrutura esperada por `services.caixa`.

    Parâmetros:
    - `vendas`: linhas de `sales` (usa `sale_date`, `total_value`).
    - `custos_fixos`: linhas de `fixed_costs` (usa `cost_date`, `amount`,
      `category`).
    - `custos_animal`: linhas de `animal_costs` (usa `cost_date`, `amount`,
      `cost_type`).
    - `compras_insumo`: linhas de `insumo_transactions` com `type == "compra"`.
       Usa `transaction_date`, `quantity` e `insumo.cost_per_unit`.
    - `contas_pagar`/`contas_receber`: parcelas (`repositories.compras`/
       `repositories.financeiro`). Usa `valor`, `vencimento`,
       `data_pagamento`/`data_recebimento` — `competencia` sai `None` de
       propósito: a receita/despesa já foi reconhecida no lançamento de origem
       (a venda, a compra); aqui é só o CRONOGRAMA de caixa dela.
       Por isso `resultado_por_competencia` nunca conta estas linhas
       (ela ignora `competencia=None`) — quem monta a lista para
       `services.caixa.fluxo_de_caixa`/`em_aberto` precisa excluir da fonte
       original (`vendas`/`compras_insumo`) o que já tem parcela aqui, senão
       o mesmo evento é contado duas vezes.

    Retorna uma lista de dicionários no formato:
      {"tipo": "receita" | "despesa", "valor": float, "categoria": str,
       "competencia": "AAAA-MM-DD" | None, "vencimento": "AAAA-MM-DD" | None,
       "pagamento": "AAAA-MM-DD" | None}
    """
    resultado = []

    if vendas:
        for v in vendas:
            norm = _normalizar_venda(v)
            if norm:
                resultado.append(norm)

    if custos_fixos:
        for cf in custos_fixos:
            norm = _normalizar_custo_fixo(cf)
            if norm:
                resultado.append(norm)

    if custos_animal:
        for ca in custos_animal:
            norm = _normalizar_custo_animal(ca)
            if norm:
                resultado.append(norm)

    if compras_insumo:
        for ci in compras_insumo:
            norm = _normalizar_compra_insumo(ci)
            if norm:
                resultado.append(norm)

    if contas_pagar:
        for cp in contas_pagar:
            norm = _normalizar_conta_pagar(cp)
            if norm:
                resultado.append(norm)

    if contas_receber:
        for cr in contas_receber:
            norm = _normalizar_conta_receber(cr)
            if norm:
                resultado.append(norm)

    return resultado
