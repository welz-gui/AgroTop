...
**Estado em 2026-08-24:** Fases A e B **e** Trilha 3 (Estoque → Financeiro → Nutrição) do
[ROADMAP](../ROADMAP.md) concluídas · **0044–0053 todas concluídas** — a Trilha 1
(API + Mobile) e a Trilha 2 (geometria/qualidade de dado) fecharam por completo (Trilha 1/2 —
...
**Trilha 2 fechada por completo em 2026-09-03** — os 4 itens do ROADMAP §5:
itens 1-3 via [PR #313](https://github.com/welz-gui/AgroTop/pull/313) (mantenedor, sem
spec, 2026-09-01) — "desenhar no mapa", "importar arquivo" e localização por propriedade
na previsão do tempo; item 4 (GPS) via [PR #322](https://github.com/welz-gui/AgroTop/pull/322)
(API) e [#325](https://github.com/welz-gui/AgroTop/pull/325) (mobile). Uma primeira
...
**0079 concluída em 2026-09-07 — [PR #370](https://github.com/welz-gui/AgroTop/pull/370).**
`services/ndvi.py` porta fielmente o cálculo da PoC (mesmas variáveis de ambiente do GDAL,
mesmas classes SCL válidas 4/5/6/7, mesmo `largest_gap()` com bordas do período) — não
reabriu nenhuma decisão já tomada pela PoC. Earth Search v1, sem chave de API (testado por
regex que não há segredo hardcoded em `services/ndvi.py`/`app.py`). Seção "🛰️ NDVI
(satélite)" em `page_lotes`, sob demanda (botão explícito, cache de 24h por
...
editar à mão — vale mencionar a próximo agente que tocar `supabase/migrations/`.

**0083 escrita em 2026-09-08** — último objetivo do relatório estratégico externo a
ganhar spec: IA como interface de consulta. Decisão de arquitetura tomada em conversa com
o usuário: **OpenRouter**, nível `:free` para começar (pesquisado antes de decidir: o
OpenRouter em si não treina modelo — é roteador —, mas modelos `:free` podem ter termos
de retenção diferentes dos pagos; tem filtro de Zero Data Retention pra quando o uso
...
| — | [0107](0107-mobile-ficha-metricas-responsivas.md) — Mobile: métricas da ficha adaptam à largura real 🏗️ | — | ✅ [#433](https://github.com/welz-gui/AgroTop/pull/433) | | 2026-09-15 |
...
> (consulta de novo após mudar status, motivo gravado no banco quando exigido).
>
> **0066 fechada em 2026-09-02 — [PR #318](https://github.com/welz-gui/AgroTop/pull/318).**
> Ponto de entrada no `AppBar` de `AnimalsPage` (`Icons.sell_outlined`, sem badge), busca
> por código, rótulos de estado/tipo conferidos byte a byte contra `app.py::_ESTADO_BRINCO`/
> `_TIPO_BRINCO` (hardcoded no Dart, mesmo padrão da 0061), transição que exige motivo
> bloqueia o botão de confirmar até preencher, transição que exige autorização aparece
> desabilitada com explicação em vez de escondida. Máquina de estados nunca reimplementada —
> só consome `transicoes_permitidas` do servidor. 12 goldens reais (10–29 KB) nos três
> temas, `build-apk` do CI (que roda `flutter analyze`+`test`+`build`) verde.
>
> **0067 fechada em 2026-09-02 — [PR #319](https://github.com/welz-gui/AgroTop/pull/319).**
...
> independentemente.
>
> **0070 fechada em 2026-09-03 — [PR #325](https://github.com/welz-gui/AgroTop/pull/325) —
> Trilha 2 inteira concluída.** Botões marcar/desfazer/recomeçar vértice, preview
> (`CustomPaint`) que só normaliza coordenadas pro canvas — confirmado que não reimplementa
> `area_hectares`/centroide/shoelace no Dart. GPS e permissões injetáveis
> (`positionProvider`/`permissionRequester`/`permissionChecker`) para teste sem hardware,
> mesmo espírito de como `ApiClient` já é injetado. Salvar desabilitado com menos de 3
> pontos, erro 422 preserva os pontos marcados na tela, pontos enviados como `[lon, lat]`
> confirmado em teste. 9 goldens reais (30-34 KB) nos 3 cenários × 3 temas. Precisou
> compactar o `AppBar` de `AnimalsPage` pra caber o 6º ícone (`IconButtonTheme` +
...
> **Histórico da revisão da PR #188 antes do merge:** a funcionalidade já estava correta —
> `GET /protocolos?animal_id=` com `dose_sugerida` usado sem recálculo no Dart, carência
> visível com ícone **e** texto, histórico de aplicações. A lacuna do golden (critério 5) foi
> [comentada na PR](https://github.com/welz-gui/AgroTop/pull/188#issuecomment-5388809084)
> antes de decidir mesclar mesmo assim — ver a nota no topo deste arquivo sobre como a
> lacuna foi tratada e como completá-la depois, se algum agente tiver Flutter disponível.
