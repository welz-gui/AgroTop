...
void main() {
      ...
      final api = _api(
        MockClient((request) async {
          ...
          if (request.method == 'POST' &&
              request.url.path == '/lotes/P01/perimetro') {
            receivedPath = request.url.path;
            receivedBody = jsonDecode(request.body) as Map<String, dynamic>;
            return _json({'ok': true, 'area_ha': 15.42, 'perimetro_m': 1600.0});
          }
  ...
  testWidgets('Critério 5: erro 422 mantém pontos marcados na tela', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(390, 844);
    tester.view.devicePixelRatio = 1;
    ...
    expect(
      find.byKey(const ValueKey('perimeter-error-message')),
      findsOneWidget,
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(
        find.byKey(const ValueKey('permission-denied-message')),
        findsOneWidget,
      );
      expect(
        find.textContaining('Permissão de localização é necessária'),
        findsOneWidget,
      );

      final retryButton = find.byKey(
        const ValueKey('request-permission-button'),
      );
  ...
  testWidgets(
    'abre PerimeterGpsPage a partir do botão no AppBar de AnimalsPage',
    (tester) async {
      final api = _api(
        MockClient((request) async {
